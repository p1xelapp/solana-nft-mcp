...
// never writes through a hard link.
{
  ...
  osx.resetKeyCache();
  globalThis.fetch = async (url) => {
    if (String(url).endsWith("/auth/keys")) return new Response(JSON.stringify({ api_key: "FRESH-KEY-0123456789", expires_at: new Date(Date.now() + 7 * 86_400_000).toISOString() }), { status: 200, headers: { "content-type": "application/json" } });
    return new Response("{}", { status: 404 });
  };
  const key = await osx.ensureKey();
  globalThis.fetch = realFetch;
  assert.strictEqual(key, "FRESH-KEY-0123456789");
  assert.strictEqual(fs.readFileSync(note, "utf8"), "keep me", "the key was written through the hard link into the note");
  assert.ok(fs.readFileSync(file, "utf8").includes("FRESH-KEY"), "the key file was not written");
  osx.resetKeyCache();
