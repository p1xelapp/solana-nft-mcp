...
- **A redirect can no longer carry a key.** Every request the server makes
  ...
  be followed with the request headers intact, OpenSea key included.
- **Recent sales validate every field.** A price is a finite amount above
  zero or null, addresses and signatures are base58 or null, a venue label is
  a plain token or `unknown`, and a fill dated ahead of this machine's clock
  is marked. Each refusal is counted in a `validation` block. The other sales
  paths already did this; this one let a -3 price through.
- **Two floors are ranked only when they are the same collection.** An
  OpenSea slug is checked against the requested collection's on-chain address
  before `get_collection_stats` compares floors; a slug that names another
  collection shows both floors with `comparable: false` and an `identity`
  block, and a caller-supplied slug that cannot be checked is shown beside,
  not ranked. `find_listings` applies the same check before joining OpenSea's
  trait floors.
- **A short credential in a named field is redacted.** `?api-key=` values and
  the userinfo password are registered from four characters; the eight-character floor
  now applies to path segments only, where a short value is as likely a route
  word. A malformed escape in a query field name no longer throws.
- **The sanitizer has no length an attacker can outrun.** Input is capped
  before any pattern runs, an opening role tag loses its bracket whether or
  not it closes, and Unicode tag characters are stripped with the other
  invisible characters.
- **A trait filter is checked, not trusted.** Each returned listing is
  compared with every requested trait; a row whose own metadata contradicts
  the filter is excluded and counted, a row that does not carry the trait is
  kept and counted as unverified. Both listing modes.
- **Caches have a ceiling.** The direct-symbol misses, symbol-check verdicts
  and account-nature reads live in bounded maps that evict past a cap and
  remove expired entries rather than keeping them.
- **The status tool never provisions.** `get_source_status` reports whether an
  OpenSea key is in hand and never requests one; the first question that needs
  OpenSea still does, unless `COLLECTOR_MCP_NO_AUTO_KEYS=1`.
- **A closed pipe is a clean exit.** A client that closes its end mid-answer
  produces exit 0 and no stack trace.
- Also: the update check accepts only a bounded semantic version; a block
  time `Date` cannot represent is a null time on a provenance row rather than
  a lost trail; the key file is read only when it is a regular file of a
  plausible size, and written through a private temporary file and a rename;
  the weekly live check runs its child with both off switches set; the
  sales-bot recipe's checklist says what a restart can repeat.
- Words: the regression suites are named by what they protect; review
  citations in comments give the date the defect was seen; the README says
  how to remove the server and what the licence does not cover; the package
  description is shorter and no longer says zero-key.
...
of resetting state; walkthrough cases that only cite another test report
REFERENCE rather than PASS, and the runner refuses to start without
`PERSONA_LIVE=1` and an explicit `PERSONA_CASES` path.

## 1.14.2 - 2026-09-15

Five gaps found by working through the walkthrough questions against live marketplaces:
...
Forty-eight walkthrough cases were run against a real server over
...
- `test/compat.mjs`: ten groups of published CLIENT constraints rather than spec
  ...
  cap, the OpenAI strict-mode schema subset, the Anthropic name regex. It is what "works on Codex, Cursor and local models" is
