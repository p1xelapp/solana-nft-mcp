...
| **CryptoSlam** - the public web API is flaky and undocumented | Each call is probed fresh in `check_sources`; a failed refresh serves the last good feed labelled stale | "CryptoSlam did not answer just now; showing the feed last seen at ..." | Only pack-pull questions are affected. If it goes for good, retire the catalog row and the two tools that read it; ~2 hours | High for a bad hour, low for a permanent loss |
